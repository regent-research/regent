...
def eval_rl(model: JatRegentModel, processor: JatProcessor, task, eval_args):
    ...
    while not all(all_dones):
        all_processed = [
            model.process(processor, **all_observations[idx], reward=all_rewards[idx], action_space=all_envs[idx].action_space, context_window=context_window, deterministic=eval_args.deterministic) for idx in range(num_envs)
        ]
...
def main():
    ...
    for task in tasks:
        if task in list(SEEN_TASK_NAME_TO_ENV_ID.keys())+list(UNSEEN_TASK_NAME_TO_ENV_ID.keys()):
            ...
            if eval_args.save_video:
                ...
                video_metadata.extend([(task, rollout_idx, score) for rollout_idx, score in enumerate(scores)])
                input_fps.extend([fps for _ in scores])
            myprint(('-'*100) + '\n\n\n')
        else:
    ...
    if eval_args.save_video and model_args.eval_fol_name is not None:
        for vid_idx, (task, rollout_idx, score) in enumerate(video_metadata):
            current_replay_path = f"{model_args.eval_fol_name}/replay_{task}_rollout{rollout_idx}_{round(score, 2)}.mp4"
            current_video = video_list[vid_idx:vid_idx+1]
            current_input_fps = input_fps[vid_idx:vid_idx+1]
            save_video_grid(current_video, current_input_fps, current_replay_path, output_fps=30, max_length_seconds=180)
            myprint(f"Saved to {current_replay_path}")
