...
from regent.utils import myprint, L2dist, atari_plotter, mujoco_plotter
...
def eval_rl(model: RandP, processor: JatProcessor, task, eval_args):
    if task.startswith("mujoco") and task in UNSEEN_TASK_NAME_TO_ENV_ID and eval_args.save_video:
        # Suppress all errors : important for mujoco plotting which has some error like warnings
        f = open(os.devnull, 'w')
        sys.stderr = f
    ...
    while not all(all_dones):
        all_processed = [
            model.process(processor, **all_observations[idx], reward=all_rewards[idx], action_space=all_envs[idx].action_space, context_window=context_window) for idx in range(num_envs)
        ]
        if eval_args.save_video and ((task.startswith("mujoco") and task in UNSEEN_TASK_NAME_TO_ENV_ID) or task.startswith("atari")):
        ...
        for idx in range(num_envs):
            if not all_dones[idx]:
                ...
                if eval_args.save_video:
                    ...
                    if task.startswith("atari"):
                        atari_plotter(task, all_envs, all_observations, retrieved_images, idx, step, eval_args)
                    elif (task.startswith("mujoco") and task in UNSEEN_TASK_NAME_TO_ENV_ID):
                        mujoco_plotter(task, all_envs, all_observations, retrieved_images, idx, step, eval_args)
                    # save frames
                    all_frames[idx].append(np.array(all_envs[idx].render(), dtype=np.uint8))
