...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, time=False, max_steps=None, sticky_p=None):
    def _thunk():
        ...
        # Sticky actions for atari
        if is_atari and sticky_p is not None:
            print('using sticky actions with p={}'.format(sticky_p))
            env = StickyActionEnv(env, sticky_p=sticky_p)
...
class StickyActionEnv(gym.Wrapper):
    # see TimeLimitMask for what I based this implementation on; Also see procgen_regent for a similar implementation
    def __init__(self, env, sticky_p):
    ...
    def step(self, action):
        if self.prev_action is not None and self.np_random.random() < self.sticky_p:
...
def make_vec_envs(env_name,
    ...
    envs = [
        make_env(env_name, seed, i, log_dir, allow_early_resets, time=time, max_steps=max_steps, sticky_p=sticky_p)
